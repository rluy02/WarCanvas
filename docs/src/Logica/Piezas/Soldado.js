...
export default class Soldado extends Pieza {
    ...
    calculaPeso() {
        ...
        const celdasVecinas = this.GetVecinos(
            this.tablero.getCelda(this.fil, this.col),
            celdasVisitadas
        );
        ...
        return peso; // FALTABA: devolver el peso calculado
    }
    ...
    detectaTipo(celda) {
        ...
        if (!celda.estaVacia() && celda.getPieza().getJugador() === 'J1') {
            // CORREGIDO: usar celda.getPieza() en lugar de vecino.getPieza()
            switch (celda.getPieza().getTipo()) {
                case 'Soldado':
                    peso += 1;
                    break; // FALTABA: break para evitar fall-through
                case 'Caballeria':
                    peso += 2;
                    break;
                case 'Comandante':
