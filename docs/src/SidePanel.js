export default class SidePanel extends Phaser.Scene {
    constructor() {
        super('SidePanel');
    }
    ...
    preload() { // Se cargan las imagenes de los dados
        for (let i = 0; i <= 6; i++) {
            this.load.image(`dice${i}`, `imgs/dice/dice${i}.png`);
        }
    }
    ...
    create() {
        ...
        this.add.rectangle(width - (sideWidth / 2), height / 2, sideWidth, height, 0xd4b37c); 

        // Elementos del Panel
        this.titleText = this.add.text(width - sideWidth + 20, 20, 'COMBATE', { // Título
            fontSize: '32px',
            fontFamily: 'Arial',
            fill: '#ffffff'
        });

        this.infoText = this.add.text(width - sideWidth + 20, 80, 'Esperando acción...', { // Texto del combate (ej: Soldado vs Caballería)
            fontSize: '16px',
            fontFamily: 'Arial',
            fill: '#ffffffff', });

        this.infoTextAttacker = this.add.text(width - sideWidth / 2, 150, 'Ataca', { // Equipo que ataca
            fontSize: '16px',
            fontFamily: 'Arial',
            fill: '#ffffffff', }).setOrigin(0.5);;

        this.infoTextDefender = this.add.text(width - sideWidth / 2, 250, 'Defiende', { // Equipo que defiende
            fontSize: '16px',
            fontFamily: 'Arial',
            fill: '#ffffffff',
        }).setOrigin(0.5);;
        ...
        this.diceImages = { // Los dos dados de cada equipo
            attacker: [
                this.add.image(width - sideWidth / 2 - 40, 200, 'dice0'),
                this.add.image(width - sideWidth / 2 + 40, 200, 'dice0')
            ],
            defender: [
                this.add.image(width - sideWidth / 2 - 40, 300, 'dice0'),
                this.add.image(width - sideWidth / 2 + 40, 300, 'dice0')
            ]
        };
        ...
        let buttonTry = this.add.text(width - sideWidth / 2, 500, 'JUGAR', { // Botón Provisional para ver si funcionan los dados
            fontSize: '30px',
            color: '#ffffff',
            fontFamily: 'Arial',
